// scripts/insert-pesticides-to-supabase.js
// data/pesticides-seed.json → static_pesticide 업로드 (upsert)
// 실행: node scripts/insert-pesticides-to-supabase.js
...
const DATA_PATH = path.join(__dirname, '..', 'data', 'pesticides-seed.json');
const rows = JSON.parse(fs.readFileSync(DATA_PATH, 'utf-8'));

// usage_fetched는 fetch resume용 플래그일 뿐 static_pesticide 컬럼이 아님
const products = rows.map(({ usage_fetched, ...rest }) => rest);
