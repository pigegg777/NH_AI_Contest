...
import { createWorkbookReviewStorageKey } from '../model/workbook-review/annotations/annotationModel';
import { SORT_DIRECTION } from '../model/workbook-review/table';
...
describe('useWorkbookReviewPipeline', () => {
  describe('annotations', () => {
    it('hydrates saved annotations and persists updates back to sessionStorage', () => {
      ...
      const { result } = renderHook(() => useWorkbookReviewPipeline(sampleRows, 'workbook-a'));
    ...
    it('keeps annotations isolated across workbook fingerprint changes', () => {
      ...
      const { result, rerender } = renderHook(
        ({ rows, fingerprint }) => useWorkbookReviewPipeline(rows, fingerprint),
    ...
    it('returns a stable empty row set without entering a render loop', () => {
      const { result, rerender } = renderHook(
        ({ fingerprint }) => useWorkbookReviewPipeline([], fingerprint),
  ...
  describe('static fertilizer merge', () => {
    it('merges static fertilizer fields into the annotated rows only after handleMerge resolves', async () => {
    ...
    it('resets merge state when the workbook fingerprint changes', async () => {
      ...
      const { result, rerender } = renderHook(
        ({ workbookFingerprint }) => useWorkbookReviewPipeline(sampleRows, workbookFingerprint),
