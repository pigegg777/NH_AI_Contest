...
} from '../../services/staticFertilizerLookupService';
import {
 ...
 */
export function useWorkbookReviewPipeline(extractedRows, workbookFingerprint) {
  ...
  const mergedRows = useMemo(() => {
    if (!isMerged || !staticFertilizerLookup) {
      return annotatedRows;
    }

    return mergeRowsWithStaticFertilizer(annotatedRows, staticFertilizerLookup);
  }, [annotatedRows, isMerged, staticFertilizerLookup]);

  const mergeStatusMessage = buildMergeStatusMessage(mergeSummary);
