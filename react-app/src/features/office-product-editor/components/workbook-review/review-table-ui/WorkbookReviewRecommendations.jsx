...
import styles from './WorkbookReviewRecommendations.module.css';
...
export function WorkbookReviewRecommendations({
  recommendations,
  analysisMode,
  ...
  return (
    <section className={`${panelStyles.panel} ${panelStyles.compactPanel}`}>
      <div className={panelStyles.panelHeader}>
        ...
        <span className={panelStyles.panelMeta}>{recommendations.length}건 추천</span>
      </div>
      ...
      {isAnalyzing ? <div className={panelStyles.statusMessage}>AI 추천 분석 중...</div> : null}
      {errorMessage ? <div className={panelStyles.errorBox}>{errorMessage}</div> : null}

      {!isAnalyzing && !errorMessage && recommendations.length === 0 ? (
        <p className={panelStyles.aiEmptyState}>아직 AI 분석 결과가 없습니다.</p>
      ) : null}
