...
function createStorageKey(officeCode) {
  ...
  return key ? `office-product-editor:market-research:${key}` : '';
}
...
export function readStoredMarketResearchState(storage, officeCode) {
...
export function writeStoredMarketResearchState(storage, officeCode, state) {
  const key = createStorageKey(officeCode);
