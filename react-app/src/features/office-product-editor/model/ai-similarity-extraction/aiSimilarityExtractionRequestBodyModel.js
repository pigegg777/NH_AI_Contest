...
import { buildAiSimilarityExtractionPrompt } from './aiSimilarityExtractionPrompt';

// Shared row cap: enforced client-side before the HTTP request is even sent
// (see analyzeAiSimilarityExtractionMatches) so a large workbook (e.g. a
...
//
// Named generically (not "similarity-extraction") because ai-bulk-note reuses
// this exact constant as its own default row cap — see
// model/ai-bulk-note/aiBulkNoteRequestBodyModel.js.
export const MAX_WORKBOOK_AI_ROWS = 500;

const AI_SIMILARITY_EXTRACTION_SCHEMA = {
  type: 'object',
...
const AI_SIMILARITY_EXTRACTION_FIELD_SERIALIZERS = {
...
function serializeWorkbookRow(row, tableNameMode) {
  ...
  for (const key of keys) {
    serializedRow[key] = AI_SIMILARITY_EXTRACTION_FIELD_SERIALIZERS[key](row);
  }
...
export function serializeRowsForAiSimilarityExtractionReview(rows, tableNameMode) {
...
export function buildAiSimilarityExtractionRequestBody({
  rows,
  tableNameMode,
  openAiModel,
  userHint,
  ...
  return {
    ...
    input: [
      {
        role: 'system',
        content: buildAiSimilarityExtractionPrompt(normalizedTableNameMode),
      },
      {
        role: 'user',
        content: JSON.stringify({
          ...
          rows: serializeRowsForAiSimilarityExtractionReview(rows, normalizedTableNameMode),
        }),
      },
    ],
    text: {
      format: {
        type: 'json_schema',
        name: 'ai_similarity_extraction_matches',
        strict: true,
        schema: AI_SIMILARITY_EXTRACTION_SCHEMA,
      },
    },
