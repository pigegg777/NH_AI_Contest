...
function createStorageKey(workbookFingerprint) {
  ...
  return key ? `office-product-editor:ai-similarity-extraction:${key}` : '';
}
...
export function readStoredAiSimilarityExtractionState(storage, workbookFingerprint) {
...
export function writeStoredAiSimilarityExtractionState(storage, workbookFingerprint, state) {
  const key = createStorageKey(workbookFingerprint);
