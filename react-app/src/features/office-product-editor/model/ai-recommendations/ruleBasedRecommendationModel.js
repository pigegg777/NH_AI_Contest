...
import { createAiRecommendation } from './aiRecommendationModel';

function buildZeroTaxHigherThanTaxRecommendations(rows) {
  return rows
    .filter(
      (row) =>
    ...
    .map((row) =>
      createAiRecommendation({
        ...
        reason: `${row.product_name || row.product_code || '행'}에서 영세단가가 과세단가보다 높게 책정되어 있습니다.`,
...
export function buildRuleBasedAiRecommendations(rows) {
  return [
    ...buildZeroTaxHigherThanTaxRecommendations(rows),
