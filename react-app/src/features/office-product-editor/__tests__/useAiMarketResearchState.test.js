...
import { useAiMarketResearchState } from '../hooks/ai-market-research/useAiMarketResearchState';
import { analyzeAiMarketResearchQuery } from '../model/ai-market-research/aiMarketResearchAnalysisModel';

vi.mock('../model/ai-market-research/aiMarketResearchAnalysisModel', () => ({
  analyzeAiMarketResearchQuery: vi.fn(),
}));
...
describe('useAiMarketResearchState', () => {
  it('runs a market research query for the given product and office', async () => {
    analyzeAiMarketResearchQuery.mockResolvedValue({ mode: 'openai', report: sampleReport });

    const { result } = renderHook(() => useAiMarketResearchState('OFF-1'));

    await act(async () => {
      await result.current.handleMarketResearch('사과 부사 5kg');
    });

    expect(analyzeAiMarketResearchQuery).toHaveBeenCalledWith('사과 부사 5kg', { officeCode: 'OFF-1', rows: [] });
  ...
  it('exposes loading state while the request is pending', async () => {
    ...
    analyzeAiMarketResearchQuery.mockReturnValue(deferred.promise);
  ...
  it('surfaces the error message when the query fails', async () => {
    analyzeAiMarketResearchQuery.mockResolvedValue({
      mode: 'error',
      report: null,
  ...
  it('persists the report to sessionStorage after a successful query', async () => {
    ...
    const stored = JSON.parse(
      globalThis.sessionStorage.getItem('office-product-editor:ai-market-research:OFF-1'),
    );
  ...
  it('restores the report from sessionStorage for the same office on mount', () => {
    ...
    expect(result.current.mode).toBe('openai');
    expect(result.current.report).toEqual(sampleReport);
    expect(analyzeAiMarketResearchQuery).not.toHaveBeenCalled();
  });

  it('does not restore another office\'s stored report', () => {
    globalThis.sessionStorage.setItem(
      'office-product-editor:ai-market-research:OFF-OTHER',
      JSON.stringify({
        version: 1,
        activeQuery: '사과 부사 5kg',
        mode: 'openai',
