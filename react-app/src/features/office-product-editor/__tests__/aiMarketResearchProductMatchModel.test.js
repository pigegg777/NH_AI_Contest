...
import { findAiMarketResearchMatchingProducts } from '../model/ai-market-research/aiMarketResearchProductMatchModel';
...
describe('findAiMarketResearchMatchingProducts', () => {
  it('ranks rows whose product_name/spec overlap the query tokens highest', () => {
    const matches = findAiMarketResearchMatchingProducts(rows, '사과 부사 5kg');
  ...
  it('falls back to zero_tax_price when tax_price is missing', () => {
    const matches = findAiMarketResearchMatchingProducts(rows, '사과 홍로');
  ...
  it('returns an empty array for a blank query or missing rows', () => {
    expect(findAiMarketResearchMatchingProducts(rows, '')).toEqual([]);
    expect(findAiMarketResearchMatchingProducts(null, '사과')).toEqual([]);
  });
  ...
  it('caps results at the given limit', () => {
    ...
    expect(findAiMarketResearchMatchingProducts(manyRows, '사과', { limit: 2 })).toHaveLength(2);
  });
});
