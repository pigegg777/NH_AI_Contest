...
describe('marketResearchStorageModel', () => {
  it('round-trips a written state through storage', () => {
    writeStoredMarketResearchState(globalThis.sessionStorage, 'OFF-1', sampleState);

    expect(readStoredMarketResearchState(globalThis.sessionStorage, 'OFF-1')).toEqual(sampleState);
  });

  it('returns null when nothing is stored for the office', () => {
    expect(readStoredMarketResearchState(globalThis.sessionStorage, 'OFF-missing')).toBe(null);
  });

  it('keeps state scoped to its own office code', () => {
    writeStoredMarketResearchState(globalThis.sessionStorage, 'OFF-1', sampleState);

    expect(readStoredMarketResearchState(globalThis.sessionStorage, 'OFF-2')).toBe(null);
  });

  it('returns null for corrupted JSON instead of throwing', () => {
    globalThis.sessionStorage.setItem('office-product-editor:market-research:OFF-1', '{not json');

    expect(readStoredMarketResearchState(globalThis.sessionStorage, 'OFF-1')).toBe(null);
  });

  it('round-trips a report-less idle state', () => {
    writeStoredMarketResearchState(globalThis.sessionStorage, 'OFF-1', {
      activeQuery: '',
      mode: 'idle',
      report: null,
      message: '',
    ...
    expect(readStoredMarketResearchState(globalThis.sessionStorage, 'OFF-1')).toEqual({
      activeQuery: '',
      mode: 'idle',
      report: null,
      message: '',
