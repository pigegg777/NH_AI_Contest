...
function buildRequest(body, { authorization = 'Bearer test-token' } = {}) {
  ...
  return new Request('https://example.com/api/market-research/analyze', {
    method: 'POST',
    headers,
...
describe('POST /api/market-research/analyze', () => {
  ...
  it('returns the fallback demo report when OpenAI fails for the pre-selected demo product', async () => {
    createClient.mockReturnValue(buildSupabaseStub());
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({ ok: false, status: 500, json: async () => ({}), text: async () => 'boom' }),
    );
    const request = buildRequest({ officeCode: 'OFF-1', productQuery: '사과 부사 5kg' });

    const response = await onRequestPost({ request, env: TEST_ENV });
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(body.report.dataFound).toBe(true);
    expect(body.report.understoodQuery).toContain('사과');
  });

  it('returns the fallback demo report when OpenAI does not respond within the timeout', async () => {
    vi.useFakeTimers();
    createClient.mockReturnValue(buildSupabaseStub());
    vi.stubGlobal('fetch', vi.fn(() => new Promise(() => {})));
    const request = buildRequest({ officeCode: 'OFF-1', productQuery: '사과 부사 5kg' });

    const responsePromise = onRequestPost({ request, env: TEST_ENV });
