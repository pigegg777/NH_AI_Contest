-- Rename static_pesticide.product_nutirent -> nutirent, and migrate the same
-- key inside office_product_datas.product_data JSON (row objects were built
-- by merging static_pesticide columns straight through as JSON keys).

alter table public.static_pesticide
  rename column product_nutirent to nutirent;

-- office_product_datas.product_data shape:
-- [ { category_name, rows: [ { ...row, product_nutirent, ... }, ... ] }, ... ]
  ...
  select jsonb_agg(
    jsonb_set(
      ...
      (
        select jsonb_agg(
          case
            when row_data ? 'product_nutirent' then
              (row_data - 'product_nutirent')
                || jsonb_build_object('nutirent', row_data -> 'product_nutirent')
            else row_data
          end
        )
        from jsonb_array_elements(entry -> 'rows') as row_data
      )
    )
  )
  from jsonb_array_elements(product_data) as entry
)
where product_data @? '$[*].rows[*].product_nutirent';

-- CREATE OR REPLACE cannot rename a column in an existing RETURNS TABLE
-- signature, so drop first.
